...
if (!adminEmail || !adminPassword) {
  console.error("ADMIN_EMAL and ADMIN_PASSWORD are required");
  process.exit(1);
}
...
class Seed {
  ...
  private async createUsersBatch(): Promise<string[]> {
    ...
    console.log(`Users created, check ./passwords/${tenant}-passwords.txt for passwords`);
  ...
  private async createIssues(
    ...
    await IssueModel.bulkCreate(data, {ignoreDuplicates: true, transaction});
  }
  ...
  private generateUserProjectAccess(projects: Project[], userIds: string[]): {projectKey: string; userId: string}[] {
    ...
    projects.forEach((project, projectIndex) => {
      const additionalUsersCount = Math.floor(Math.random() * 4) + 5;
